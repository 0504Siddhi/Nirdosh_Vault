...
function normalizeDocumentType(
  ...
  const normalized =
    normalizeLookupValue(documentType);
...
function normalizeScenario(
  ...
  return normalizeLookupValue(scenario);
}

function buildOfficialSources(
  raw: RawCorrectionRule,
): OfficialSource[] {
  if (Array.isArray(raw.official_sources)) {
    return raw.official_sources
      ...
      .map(source => {
        ...
        return {
          authority:
            asString(item.authority) ||
            asString(raw.authority),
          title: asString(item.title),
          url: asString(item.url),
          exact_support: asString(
            item.exact_support,
          ),
        };
      })
  ...
  /*
   * Existing JSON may store either:
   * 1. a URL in official_source, or
   * 2. an official source title in official_source.
   *
   * Do not blindly treat a title as a URL.
   */
  const isUrl =
...
function toCorrectionRule(
  ...
  return {
    ...
    field_key: normalizeFieldKey(
      asString(raw.field_key),
    ),

    scenario: normalizeScenario(
      asString(raw.scenario),
    ),

    priority: asNumber(raw.priority),

    trigger_source: asString(
      raw.trigger_source,
    ),

    requires_user_input: asBoolean(
      raw.requires_user_input,
    ),
    ...
    official_sources:
      buildOfficialSources(raw),

    source_checked_date: asString(
      raw.source_checked_date,
    ),

    expires_for_review_on:
      asOptionalString(
        raw.expires_for_review_on,
      ),

    disclaimer:
      asString(raw.disclaimer) ||
      LEGAL_BOUNDARY,
  };
}

const parsedRuleData =
  ruleData as CorrectionRulesFile;

const rules: CorrectionRule[] = (
  parsedRuleData.rules ?? []
)
...
function isExpired(
  ...
  /*
   * Date-only strings can behave differently across time zones.
   * Treat the rule as valid through the end of the stated day.
   */
  const dateOnlyPattern =
    /^\d{4}-\d{2}-\d{2}$/;

  const expiryDate = dateOnlyPattern.test(
    expiresOn,
  )
    ? new Date(
  ...
  if (
    ...
    /*
     * Invalid expiry data should not silently make
     * an unverified rule appear active.
     */
    return true;
  }
...
function isActiveRule(
  ...
  if (
    rule.rule_status === 'unverified'
  ) {
    return false;
  }

  return !isExpired(
    rule.expires_for_review_on,
  );
}
...
function findBestRule(
  ...
  const normalizedFieldKey =
    normalizeFieldKey(result.fieldKey);

  const normalizedScenario =
    normalizeScenario(result.scenario);
  ...
  /*
   * Safe fallback:
   * Permit a generic scenario only when the JSON
   * explicitly contains one for this document and field.
   */
...
export function buildCorrectionKit(
  ...
  if (
    ...
    return {
      ...base,
      guide_status:
        'unsupported_rule' as GuideStatus,
  ...
  if (!documentType) {
    return {
      ...base,
      guide_status:
        'requires_user_input' as GuideStatus,
      selected_rule_id: null,
      correction_guide: null,
      official_evidence: [],
      next_action:
  ...
  return {
    ...
    correction_guide: {
      ...
      steps:
        rule.recommended_steps,

      supporting_document_categories:
        rule.supporting_document_categories,

      human_review_required:
        rule.human_review_required,

      source_checked_date:
        rule.source_checked_date,
  ...
  return fieldResults
    ...
    .map(result => {
      ...
      return {
        ...
        /*
         * Rules stay empty until a correction target
         * document is selected or determined by the route.
         * This prevents the service from treating the
         * majority value as legal truth automatically.
         */
        rules: [],
